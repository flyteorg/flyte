...
Build from a checkout with ``make -C flytestdlib compile``; the binary is
``flytestdlib/bin/pflags``. Add that directory to ``PATH`` before running
``go generate``.

Getting Started
^^^^^^^^^^^^^^^
 - From the repository root, run ``go install ./flytestdlib/cli/pflags``.
...
This generates two files (struct_name_flags.go and struct_name_flags_test.go). If you open those, you will notice that all generated flags default to empty/zero values and no usage strings. That behavior can be customized using ``pflag`` tag.

...
func discoverFieldsRecursive(ctx context.Context, workingDirPkg string, typ *types.Named, defaultValueAccessor, fieldPath string, bindDefaultVar bool) ([]FieldInfo, []PFlagValueType, error) {
	...
	for i := 0; i < st.NumFields(); i++ {
		...
		// Go 1.23+ materializes type aliases (gotypesalias=1); unwrap them so
		// aliased types (e.g. `type Foo = string`) generate as their RHS type.
...
// NewGenerator initializes a PFlagProviderGenerator for pflags files for targetTypeName struct under pkg. If pkg is not
// filled in, it's assumed to be current package (which is expected to be the common use case when invoking pflags
// from go:generate comments)

...
func (cfg TestType) GetPFlagSet(prefix string) *pflag.FlagSet {
	...
	cmdFlags.String(fmt.Sprintf("%v%v", prefix, "storage.type"), DefaultTestType.StorageConfig.Type, "Sets the type of storage [s3/minio/local/mem/stow/redis].")
	cmdFlags.String(fmt.Sprintf("%v%v", prefix, "storage.stow.kind"), DefaultTestType.StorageConfig.Stow.Kind, "Kind of Stow backend to use. Refer to github/flyteorg/stow")
	cmdFlags.StringToString(fmt.Sprintf("%v%v", prefix, "storage.stow.config"), DefaultTestType.StorageConfig.Stow.Config, "Configuration for stow backend. Refer to github/flyteorg/stow")
	...
	cmdFlags.Int(fmt.Sprintf("%v%v", prefix, "storage.defaultHttpClient.maxIdleConns"), DefaultTestType.StorageConfig.DefaultHTTPClient.MaxIdleConns, "Maximum number of idle connections across all hosts. Zero means use the http.DefaultTransport value.")
	cmdFlags.Int(fmt.Sprintf("%v%v", prefix, "storage.defaultHttpClient.maxIdleConnsPerHost"), DefaultTestType.StorageConfig.DefaultHTTPClient.MaxIdleConnsPerHost, "Maximum number of idle connections per host. Zero means use the http.DefaultTransport value.")
	cmdFlags.Int(fmt.Sprintf("%v%v", prefix, "storage.defaultHttpClient.maxConnsPerHost"), DefaultTestType.StorageConfig.DefaultHTTPClient.MaxConnsPerHost, "Maximum number of connections per host; new requests block at the limit. Zero means no limit.")
	cmdFlags.String(fmt.Sprintf("%v%v", prefix, "storage.defaultHttpClient.idleConnTimeout"), DefaultTestType.StorageConfig.DefaultHTTPClient.IdleConnTimeout.String(), "Maximum amount of time an idle connection remains open. Zero means use the http.DefaultTransport value.")

...
func (_m *CreateBatchesFunc) Execute(ctx context.Context, snapshot []cache.ItemWrapper) ([]cache.Batch, error) {
	...
	var r0 []cache.Batch
	var r1 error
	if rf, ok := ret.Get(0).(func(context.Context, []cache.ItemWrapper) ([]cache.Batch, error)); ok {
		return rf(ctx, snapshot)
	}
	if rf, ok := ret.Get(0).(func(context.Context, []cache.ItemWrapper) []cache.Batch); ok {
		r0 = rf(ctx, snapshot)
	} else {
		if ret.Get(0) != nil {

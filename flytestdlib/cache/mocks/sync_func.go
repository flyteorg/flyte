...
func (_m *SyncFunc) Execute(ctx context.Context, batch cache.Batch) ([]cache.ItemSyncResponse, error) {
	...
	var r1 error
	if rf, ok := ret.Get(0).(func(context.Context, cache.Batch) ([]cache.ItemSyncResponse, error)); ok {
		return rf(ctx, batch)
	}
	if rf, ok := ret.Get(0).(func(context.Context, cache.Batch) []cache.ItemSyncResponse); ok {
		r0 = rf(ctx, batch)
	} else {
		if ret.Get(0) != nil {
	...
	if rf, ok := ret.Get(1).(func(context.Context, cache.Batch) error); ok {
		r1 = rf(ctx, batch)
	} else {
		r1 = ret.Error(1)
	}
...
func (_c *SyncFunc_Execute_Call) Run(run func(ctx context.Context, batch cache.Batch)) *SyncFunc_Execute_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run(args[0].(context.Context), args[1].(cache.Batch))
	})
	return _c
}

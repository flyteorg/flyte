...
func TestGetAuthenticationDialOptionClientSecret(t *testing.T) {
	...
	t.Run("legal-no-external-calls", func(t *testing.T) {
		mockAuthClient := new(mocks.AuthMetadataServiceClient)
		mockAuthClient.On("GetOAuth2Metadata", mock.Anything, mock.Anything).Return(nil, errors.New("unexpected call to get oauth2 metadata"))
		mockAuthClient.On("GetPublicClientConfig", mock.Anything, mock.Anything).Return(nil, errors.New("unexpected call to get public client config"))
		var adminCfg Config
	...
	t.Run("error during oauth2Metatdata", func(t *testing.T) {
		mockAuthClient := new(mocks.AuthMetadataServiceClient)
		mockAuthClient.On("GetOAuth2Metadata", mock.Anything, mock.Anything).Return(nil, fmt.Errorf("failed"))
		dialOption, err := getAuthenticationDialOption(ctx, adminServiceConfig, nil, mockAuthClient)
		assert.Nil(t, dialOption)
		assert.NotNil(t, err)
	})
	t.Run("error during public client config", func(t *testing.T) {
		...
		mockAuthClient.On("GetPublicClientConfig", mock.Anything, mock.Anything).Return(nil, failedPublicClientConfigLookup)
		var adminCfg Config
		err := copier.Copy(&adminCfg, adminServiceConfig)
		assert.NoError(t, err)
		adminCfg.TokenURL = "http://localhost:1000/api/v1/token"
	...
	t.Run("error during flyte client", func(t *testing.T) {
		...
		mockAuthClient := new(mocks.AuthMetadataServiceClient)
		mockAuthClient.On("GetOAuth2Metadata", mock.Anything, mock.Anything).Return(metadata, nil)
		mockAuthClient.On("GetPublicClientConfig", mock.Anything, mock.Anything).Return(nil, fmt.Errorf("failed"))
		dialOption, err := getAuthenticationDialOption(ctx, adminServiceConfig, nil, mockAuthClient)
		assert.Nil(t, dialOption)
		assert.NotNil(t, err)
	})

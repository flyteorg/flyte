...
func Test_newAuthInterceptor(t *testing.T) {
	...
	t.Run("Other Error", func(t *testing.T) {
		...
		m.On("GetOAuth2Metadata", mock.Anything, mock.Anything).Return(&service.OAuth2MetadataResponse{
		...
		m.On("GetPublicClientConfig", mock.Anything, mock.Anything).Return(&service.PublicClientAuthConfigResponse{
			Scopes: []string{"all"},
		}, nil)
		...
		mockTokenCache.On("GetToken").Return(tokenData, nil)
		mockTokenCache.On("SaveToken", mock.Anything).Return(nil)
	...
	t.Run("Unauthenticated first time, succeed the second time", func(t *testing.T) {
		...
		p := NewPerRPCCredentialsFuture()
		c := &mocks.TokenCache{}
		c.On("GetToken").Return(nil, nil)
		c.On("TryLock").Return(true)
		c.On("SaveToken", mock.Anything).Return(nil)
		c.On("CondBroadcast").Return()
		c.On("Unlock").Return()
		c.On("PurgeIfEquals", mock.Anything).Return(true, nil)
		interceptor := NewAuthInterceptor(&Config{
			Endpoint:              config.URL{URL: *u},
			UseInsecureConnection: true,
...
func TestNewAuthInterceptorAndMaterialize(t *testing.T) {
	t.Run("No oauth2 metadata endpoint or Public client config lookup", func(t *testing.T) {
		...
		c := &mocks.TokenCache{}
		c.On("GetToken").Return(fakeToken, nil)
		c.On("SaveToken", mock.Anything).Return(nil)
		m := &adminMocks.AuthMetadataServiceServer{}
		m.On("GetOAuth2Metadata", mock.Anything, mock.Anything).Return(nil, errors.New("unexpected call to get oauth2 metadata"))
		m.On("GetPublicClientConfig", mock.Anything, mock.Anything).Return(nil, errors.New("unexpected call to get public client config"))
		s := newAuthMetadataServer(t, grpcPort, httpPort, m)
		ctx := context.Background()
		assert.NoError(t, s.Start(ctx))
		defer s.Close()
	...
	t.Run("Failed to fetch client metadata", func(t *testing.T) {
		...
		m.On("GetPublicClientConfig", mock.Anything, mock.Anything).Return(nil, failedPublicClientConfigLookup)
		s := newAuthMetadataServer(t, grpcPort, httpPort, m)
		ctx := context.Background()
		assert.NoError(t, s.Start(ctx))
		defer s.Close()

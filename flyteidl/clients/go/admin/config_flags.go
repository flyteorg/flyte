...
func (cfg Config) GetPFlagSet(prefix string) *pflag.FlagSet {
	...
	cmdFlags.Int(fmt.Sprintf("%v%v", prefix, "maxMessageSizeBytes"), defaultConfig.MaxMessageSizeBytes, "The max size in bytes for incoming gRPC messages")
	cmdFlags.String(fmt.Sprintf("%v%v", prefix, "authType"), defaultConfig.AuthType.String(), "Type of OAuth2 flow used for communicating with admin.ClientSecret, Pkce, ExternalCommand, DeviceFlow are valid values")
	...
	cmdFlags.String(fmt.Sprintf("%v%v", prefix, "authorizationUrl"), defaultConfig.AuthorizationURL, "OPTIONAL: Authorization endpoint to use for the Pkce auth type instead of the one admin advertises. Requires tokenUrl,  clientId and scopes.")
	cmdFlags.String(fmt.Sprintf("%v%v", prefix, "deviceAuthorizationUrl"), defaultConfig.DeviceAuthorizationURL, "OPTIONAL: Device authorization endpoint to use for the DeviceFlow auth type instead of the one admin advertises. Requires tokenUrl,  clientId and scopes.")
	cmdFlags.String(fmt.Sprintf("%v%v", prefix, "tokenType"), defaultConfig.TokenType, "OPTIONAL: Scheme used to send the token to admin: Bearer (default) or IDToken. IDToken sends the OIDC id_token instead of the access token.")

...
- Each Flyte installation type should have separate `values-*.yaml` file: for sandbox, EKS, etc. The configuration in `values.yaml` and the choosen config `values-*.yaml` are merged when generating the deployment manifest.

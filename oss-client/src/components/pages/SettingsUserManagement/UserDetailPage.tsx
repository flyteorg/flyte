...
export function SettingsUserDetailPage({ principal: _principal }: { principal: string }) {
  const searchParams = useSearchParams()
  const returnTo = searchParams.get('returnTo')

  const backUrl = returnTo
    ? `/settings/user-management?returnTo=${encodeURIComponent(returnTo)}`
...
export default SettingsUserDetailPage

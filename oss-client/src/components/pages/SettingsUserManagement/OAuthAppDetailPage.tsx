...
export function SettingsOAuthAppDetailPage({
  clientId: _clientId,
}: {
  clientId: string
}) {
  ...
  const backUrl = returnTo
    ? `/settings/user-management?tab=apps&returnTo=${encodeURIComponent(returnTo)}`
    : '/settings/user-management?tab=apps'
...
export default SettingsOAuthAppDetailPage

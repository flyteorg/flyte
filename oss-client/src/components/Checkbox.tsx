...
const base = [
  ...
  // Background color when checked (only if color is provided)
  'has-[--checkbox-checked-bg]:group-data-checked:before:bg-(--checkbox-checked-bg)',
  // Background color is moved to control and shadow is removed in dark mode so hide `before` pseudo
  'dark:before:hidden',
  // Background color applied to control in dark mode (only if color is provided)
  'dark:bg-white/5 has-[--checkbox-checked-bg]:dark:group-data-checked:bg-(--checkbox-checked-bg)',
  // Border
  'border border-zinc-950/15 group-data-checked:border-transparent group-data-hover:group-data-checked:border-transparent group-data-hover:border-zinc-950/30',
  'has-[--checkbox-checked-border]:group-data-checked:bg-(--checkbox-checked-border)',
  'dark:border-white/15 dark:group-data-checked:border-white/5 dark:group-data-hover:group-data-checked:border-white/5 dark:group-data-hover:border-white/30',
  // Inner highlight shadow
  'after:absolute after:inset-0 after:rounded-[calc(0.3125rem-1px)] after:shadow-[inset_0_1px_--theme(--color-white/15%)]',
  'dark:after:-inset-px dark:after:hidden dark:after:rounded-[0.3125rem] has-[--checkbox-checked-bg]:dark:group-data-checked:after:block',
  // Focus ring
  ...
  'dark:group-data-disabled:border-white/20 dark:group-data-disabled:bg-white/2.5 dark:group-data-disabled:[--checkbox-check:var(--color-white)]/50 has-[--checkbox-checked-bg]:dark:group-data-checked:group-data-disabled:after:hidden',
  // Forced colors mode
    ...
    >
      <span
        className={clsx([
          ...
          color
            ? colors[color]
            : '[--checkbox-check:var(--color-zinc-950)] [--checkbox-checked-border:var(--system-gray-4)] dark:[--checkbox-check:var(--color-white)]',
        ])}
      >
        <svg
          className="size-3 stroke-(--checkbox-check) opacity-0 group-data-checked:opacity-100 sm:h-3 sm:w-3"
          viewBox="0 0 14 14"
          fill="none"
        >
          {/* Checkmark icon */}

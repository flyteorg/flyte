...
type Config struct {
	GrpcPort                 int  `json:"grpcPort" pflag:",On which grpc port to serve Cache Service"`
	GrpcServerReflection     bool `json:"grpcServerReflection" pflag:",Enable GRPC Server Reflection"`
	HTTPPort                 int  `json:"httpPort" pflag:",On which http port to serve CacheService"`
	Secure                   bool `json:"secure" pflag:",Whether to run CacheService in secure mode or not"`

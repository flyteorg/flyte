	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.expectCreateCalled {
				mockReservationStoreClient.AssertCalled(t, "Create", mock.Anything, mock.Anything)
			} else {
				mockReservationStoreClient.AssertNotCalled(t, "Create", mock.Anything, mock.Anything)
			}
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			mockReservationStoreClient.OnDeleteMatch(
				...
				mock.MatchedBy(func(key string) bool {
					assert.EqualValues(t, sampleKey, key)
					return true
				}), mock.MatchedBy(func(ownerId string) bool {
					assert.EqualValues(t, sampleOwner, ownerId)
					return true
				}),

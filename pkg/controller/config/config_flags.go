...
func (cfg Config) GetPFlagSet(prefix string) *pflag.FlagSet {
	...
	cmdFlags.StringSlice(fmt.Sprintf("%v%v", prefix, "include-shard-key-label"), []string{}, "Include the specified shard key label in the k8s FlyteWorkflow CRD label selector")
	cmdFlags.StringSlice(fmt.Sprintf("%v%v", prefix, "exclude-shard-key-label"), []string{}, "Exclude the specified shard key label from the k8s FlyteWorkflow CRD label selector")
	cmdFlags.StringSlice(fmt.Sprintf("%v%v", prefix, "include-project-label"), []string{}, "Include the specified project label in the k8s FlyteWorkflow CRD label selector")

...
func (s *subworkflowHandler) DoInlineSubWorkflow(ctx context.Context, nCtx handler.NodeExecutionContext, w v1alpha1.ExecutableWorkflow,
	...
	if state.HasFailed() {
		if w.GetOnFailureNode() != nil {
			...
			return handler.DoTransition(handler.TransitionTypeEphemeral, handler.PhaseInfoFailure(string(errors.SubWorkflowExecutionFailed), state.Err.Error(), nil)), err
		}
	...
	if state.IsComplete() {
		...
		if outputBindings := w.GetOutputBindings(); len(outputBindings) > 0 {
			...
			if endNodeStatus == nil {
				return handler.DoTransition(handler.TransitionTypeEphemeral, handler.PhaseInfoFailure(string(errors.SubWorkflowExecutionFailed), "No end node found in subworkflow.", nil)), err
			}
...
func (s *subworkflowHandler) StartSubWorkflow(ctx context.Context, nCtx handler.NodeExecutionContext) (handler.Transition, error) {
	...
	if len(nodeStatus.GetDataDir()) == 0 {
		...
		if startStatus.HasFailed() {
			...
			return handler.DoTransition(handler.TransitionTypeEphemeral, handler.PhaseInfoFailure(string(errorCode), startStatus.Err.Error(), nil)), nil
		}
	}

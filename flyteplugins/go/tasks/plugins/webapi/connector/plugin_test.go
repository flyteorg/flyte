...
func TestPlugin(t *testing.T) {
	...
	t.Run("test getFinalConnector", func(t *testing.T) {
		spark := &connectorPb.TaskCategory{Name: "spark", Version: defaultTaskTypeVersion}
...
func getMockMetadataServiceClient() *connectorMocks.ConnectorMetadataServiceClient {
	...
	mockRequest := &connectorPb.ListConnectorsRequest{}
	supportedTaskCategories := make([]*connectorPb.TaskCategory, 3)
	supportedTaskCategories[0] = &connectorPb.TaskCategory{Name: "task1", Version: defaultTaskTypeVersion}
	supportedTaskCategories[1] = &connectorPb.TaskCategory{Name: "task2", Version: defaultTaskTypeVersion}
	supportedTaskCategories[2] = &connectorPb.TaskCategory{Name: "task3", Version: defaultTaskTypeVersion}
	mockResponse := &connectorPb.ListConnectorsResponse{
		Connectors: []*connectorPb.Connector{
			{
				Name:                    "test-agent",
...
func TestInitializeConnectorRegistry(t *testing.T) {
	connectorClients := make(map[string]connectorPb.AsyncConnectorServiceClient)
	connectorMetadataClients := make(map[string]connectorPb.ConnectorMetadataServiceClient)

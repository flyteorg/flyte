...
func TestGetAndSetConfig(t *testing.T) {
	...
	cfg.ConnectorForTaskTypes = map[string]string{"task_type_1": "connector_1"}
	err := SetConfig(&cfg)
	assert.NoError(t, err)
	assert.Equal(t, &cfg, GetLegacyConfig())
}

func TestDefaultConnectorConfig(t *testing.T) {
	cfg := defaultLegacyConfig
	...
	assert.Empty(t, cfg.ConnectorForTaskTypes)

...
func TestGetEventInfo(t *testing.T) {
	...
	info, err := getEventInfoForSpark(taskCtx, dummySparkApplication(sparkOp.ApplicationStateRunning))
	...
	info, err = getEventInfoForSpark(taskCtx, dummySparkApplication(sparkOp.ApplicationStateSubmitted))
	assert.NoError(t, err)
	...
	info, err = getEventInfoForSpark(taskCtx, dummySparkApplication(sparkOp.ApplicationStateFailed))
	assert.NoError(t, err)
	assert.Len(t, info.Logs, 5)

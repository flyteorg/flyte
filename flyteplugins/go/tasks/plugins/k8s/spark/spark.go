...
func getSparkConfig(taskCtx pluginsCore.TaskExecutionContext, sparkJob *plugins.SparkJob) map[string]string {
	...
	// Set pod limits.
	if len(sparkConfig[SparkDriverCoreLimitKey]) == 0 {
		// spark.kubernetes.driver.request.cores takes precedence over spark.driver.cores
		if len(sparkConfig[SparkDriverCoreRequestKey]) != 0 {
			sparkConfig[SparkDriverCoreLimitKey] = sparkConfig[SparkDriverCoreRequestKey]
		} else if len(sparkConfig["spark.driver.cores"]) != 0 {
			sparkConfig[SparkDriverCoreLimitKey] = sparkConfig["spark.driver.cores"]
		}
	}

	if len(sparkConfig[SparkExecutorCoreLimitKey]) == 0 {
		// spark.kubernetes.executor.request.cores takes precedence over spark.executor.cores
		if len(sparkConfig[SparkExecutorCoreRequestKey]) != 0 {
			sparkConfig[SparkExecutorCoreLimitKey] = sparkConfig[SparkExecutorCoreRequestKey]
		} else if len(sparkConfig["spark.executor.cores"]) != 0 {
			sparkConfig[SparkExecutorCoreLimitKey] = sparkConfig["spark.executor.cores"]
		}
	}
...
func createSparkApplication(sparkJob *plugins.SparkJob, sparkConfig map[string]string, driverSpec *driverSpec,
	...
	sparkVersion := "3.0.0" // Default version
	if v, ok := sparkConfig["spark.version"]; ok {
		sparkVersion = v
	}

...
var _ k8s.ChildPodDiscovery = mpiOperatorResourceHandler{}
...
// ChildPods implements k8s.ChildPodDiscovery. The pods that run the task are the launcher
// and worker pods the training operator expands from the templates this plugin built,
// which the framework tracks nothing of, since it tracks the MPIJob.
func (mpiOperatorResourceHandler) ChildPods(
	ctx context.Context,
	taskCtx pluginsCore.TaskExecutionMetadata,
	resource client.Object,
) (labels.Selector, error) {
	if _, ok := resource.(*kubeflowv1.MPIJob); !ok {
		return nil, flyteerr.Errorf(flyteerr.BadTaskSpecification, "expected an MPIJob, got %T", resource)
	}

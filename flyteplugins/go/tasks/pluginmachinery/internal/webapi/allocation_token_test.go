...
func Test_allocateToken(t *testing.T) {
	...
	rm.EXPECT().AllocateResource(mock.Anything, core.ResourceNamespace("ns"), "abc", mock.Anything).Return(core.AllocationStatusGranted, nil)
	...
	t.Run("Allocation Failed", func(t *testing.T) {
		...
		rm := &mocks2.ResourceManager{}
		rm.EXPECT().AllocateResource(mock.Anything, core.ResourceNamespace("ns"), "abc2", mock.Anything).Return(core.AllocationStatusExhausted, nil)
...
func Test_releaseToken(t *testing.T) {
	...
	clck := testing2.NewFakeClock(time.Now())
	...
	rm := &mocks2.ResourceManager{}
	rm.EXPECT().ReleaseResource(mock.Anything, core.ResourceNamespace("ns"), "abc").Return(nil)

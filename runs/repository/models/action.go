...
type Action struct {
	...
	// CreatedBy is the OIDC subject of the run's creator, captured from the auth headers
	// the load balancer forwards. Stored as a queryable, indexed column (filter/sort by
	// owner). The display identity (name/email) is resolved from this subject at read time,
	// so it never goes stale. Surfaced as ActionMetadata.executed_by. NULL for runs created

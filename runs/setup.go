...
func Setup(ctx context.Context, sc *app.SetupContext) error {
	...
	dataProxyURL := cfg.DataProxyServiceURL
	if sc.BaseURL != "" {
		dataProxyURL = sc.BaseURL
	}
	dataProxyClient := dataproxyconnect.NewDataProxyServiceClient(http.DefaultClient, dataProxyURL)
	runLogsSvc := service.NewRunLogsService(dataProxyClient)

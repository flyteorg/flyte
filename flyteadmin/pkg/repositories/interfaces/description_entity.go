...
type DescriptionEntityRepoInterface interface {
	...
	Get(ctx context.Context, id *core.Identifier) (models.DescriptionEntity, error)
	// List Returns DescriptionEntity matching query parameters. A limit must be provided for the results page size
	List(ctx context.Context, entity common.Entity, input ListResourceInput) (DescriptionEntityCollectionOutput, error)
}

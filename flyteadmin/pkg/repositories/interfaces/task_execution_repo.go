...
type TaskExecutionRepoInterface interface {
	...
	Create(ctx context.Context, id *core.TaskExecutionIdentifier, input models.TaskExecution) error
	// Updates an existing task execution in the database store with all non-empty fields in the input.
	Update(ctx context.Context, id *core.TaskExecutionIdentifier, execution models.TaskExecution) error
	// Returns a matching execution if it exists.
	Get(ctx context.Context, id *core.TaskExecutionIdentifier) (models.TaskExecution, error)

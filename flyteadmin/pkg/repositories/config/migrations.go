...
var UnionMigrations = []*gormigrate.Migration{
	...
	{
		ID: "2024-03-21-execution-idx_org_project_domain_phase_execution_created_at",
		Migrate: func(tx *gorm.DB) error {
			type Execution struct {
				ID           uint       `gorm:"index;autoIncrement;not null"`
				CreatedAt    time.Time  `gorm:"type:time"`
				...
				LaunchEntity string
				// Tags associated with the execution
				Tags []models.AdminTag `gorm:"many2many:execution_admin_tags;"`
			}
			return tx.AutoMigrate(&Execution{})
		},
		Rollback: func(tx *gorm.DB) error {
...
var ContinuedMigrations = []*gormigrate.Migration{
	...
	},
	{
		...
		Migrate: func(tx *gorm.DB) error {
			type ExecutionKey struct {
				Org     string `gorm:"primary_key;column:execution_org;size:255"`
			...
			return tx.Transaction(func(_ *gorm.DB) error {
				...
				sql := "INSERT INTO execution_tags (execution_org, execution_project, execution_domain, execution_name, created_at, updated_at, deleted_at, key, value)" +
					" SELECT execution_org, execution_project, execution_domain, execution_name, created_at, updated_at, deleted_at, name as key, '' as value" +

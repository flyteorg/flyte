	...
	DefaultConfig = &Config{
		...
		AppAuth: OAuth2Options{
			ExternalAuthServer: ExternalAuthorizationServer{
				RetryAttempts:          5,
				RetryDelayMilliseconds: config.Duration{Duration: 1000 * time.Millisecond},
			},
...
type ExternalAuthorizationServer struct {
	...
	RetryDelayMilliseconds config.Duration `json:"retryDelayMilliseconds" pflag:", Optional, Duration in milliseconds to wait between retries"`
}

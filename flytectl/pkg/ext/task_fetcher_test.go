...
func TestFetchAllVerOfTask(t *testing.T) {
	getTaskFetcherSetup()
	adminClient.EXPECT().ListTasks(mock.Anything, mock.Anything).Return(taskListResponse, nil)
...
func TestFetchTaskVersion(t *testing.T) {
	getTaskFetcherSetup()
	adminClient.EXPECT().GetTask(mock.Anything, mock.Anything).Return(task1, nil)
...
func TestFetchAllVerOfTaskError(t *testing.T) {
	getTaskFetcherSetup()
	adminClient.EXPECT().ListTasks(mock.Anything, mock.Anything).Return(nil, fmt.Errorf("failed"))
	_, err := adminFetcherExt.FetchAllVerOfTask(ctx, "taskName", "project", "domain", taskFilter)

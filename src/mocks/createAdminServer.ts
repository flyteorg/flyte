    ...
    return (req, res, ctx) => {
        try {
            ...
            if (e instanceof RequestError) {
                ...
                return res(ctx.status(500), ctx.text(`Unexpected error: ${e}`));
        }
    };
}
    ...
    const getTaskExecutionChildListHandler = adminEntityHandler({
        ...
        getDataForRequest: req => {
            ...
            return {
                nodeExecutions: data.map(nodeExecutionId => {
                    try {
                        const execution = getItem<Admin.INodeExecution>(
                            entityMap,
                            [EntityType.NodeExecution, nodeExecutionId]
                        );
    ...
    return {
        ...
        server: {
            ...
            insertWorkflow: workflow =>
                insertItem(
                    entityMap,
                    [EntityType.Workflow, workflowIdentifier(workflow.id)],
                    workflow
                ),

...
describe('AdminEntity/utils', () => {
    ...
    describe('decodeProtoResponse', () => {
        it('provides value as a Uint8Array', () => {
            ...
            expect(messageType.decode).toHaveBeenCalledWith(
    ...
    describe('createPaginationTransformer', () => {
        it('extracts the correct property', () => {
            ...
            const transformer = createPaginationTransformer<
                string,
                typeof data
            >('items');

            expect(transformer(data)).toEqual(
                expect.objectContaining({ entities: data.items })
            );
        });
        ...
        it('forwards pagination token', () => {
            ...
            expect(transformer(data)).toEqual(
                expect.objectContaining({ token: data.token })
            );
        });
    });
});

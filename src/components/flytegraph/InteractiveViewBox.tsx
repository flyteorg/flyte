...
function translateViewBox(
    ...
    const x = Math.min(
        Math.max(0, viewBox.x - translateX),
        viewBox.naturalWidth - viewBox.width
    );
    const y = Math.min(
        Math.max(0, viewBox.y - translateY),
        viewBox.naturalHeight - viewBox.height
    );
    return { ...viewBox, x, y };
}
...
function scaleViewBox(viewBox: ViewBoxRect, scaleDelta: number): ViewBoxRect {
    ...
    const height =
        scaleDelta < 1
            ? Math.max(
                  minScale * viewBox.naturalHeight,

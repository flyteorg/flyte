...
function renderListItem(value: any, index: number) {
  return <RenderedProtobufValue label={index} value={value as ProtobufValue} />;
}
    ...
    case 'listValue': {
      ...
      return (
        <>
          ...
          <PrintList values={list.values} renderValue={renderListItem} />
        </>
      );
    }
    case 'structValue':
      return (
        <>
          <ValueLabel label={label} />
          <ProtobufStructValue struct={value.structValue as ProtobufStruct} />
        </>
      );
    default:

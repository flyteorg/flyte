...
describe('compareTimestampsAscending', () => {
    ...
    const cases: [Protobuf.ITimestamp, Protobuf.ITimestamp, -1 | 0 | 1][] = [
        ...
        [
            { seconds: long(10), nanos: 0 },
            { seconds: long(9), nanos: 1e9 - 1 },
            1
        ],
        [
            { seconds: long(9), nanos: 1e9 - 1 },
    ...
    cases.forEach(([left, right, expected]) =>
        it(`should return ${expected} when comparing ${obj(left)}, to ${obj(
            right
        )}`, () =>
...
describe('createCorsProxyURL', () => {
    ...
    it('should correctly construct the path', () => {
        expect(createCorsProxyURL(url)).toBe(
            `${window.location.origin}${corsPrefix}/${url}`
        );
    });
    ...
    it('should prepend the base url if provided', () => {
        ...
        expect(createCorsProxyURL(url)).toBe(
            `${window.location.origin}/console${corsPrefix}/${url}`
        );
    });
});

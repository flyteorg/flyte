...
/* 
AdminServiceApiService
Update the state of an existing workflow definition.
 * @param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 * @param idProject Name of the project the resource belongs to.
 * @param idDomain Name of the domain the resource belongs to. A domain can be considered as a subset within a specific project.
 * @param idName User provided value for the resource.
...
*/
func (a *AdminServiceApiService) UpdateWorkflow(ctx context.Context, idProject string, idDomain string, idName string, idVersion string, body AdminWorkflowUpdateRequest) (AdminWorkflowUpdateResponse, *http.Response, error) {
	var (
		localVarHttpMethod = strings.ToUpper("Put")
		localVarPostBody   interface{}
		localVarFileName   string
		localVarFileBytes  []byte
		localVarReturnValue AdminWorkflowUpdateResponse
	)
	...
	if localVarHttpResponse.StatusCode >= 300 {
		...
		if localVarHttpResponse.StatusCode == 200 {
			var v AdminWorkflowUpdateResponse
			err = a.client.decode(&v, localVarBody, localVarHttpResponse.Header.Get("Content-Type"));
				if err != nil {
					newErr.error = err.Error()
					return localVarReturnValue, localVarHttpResponse, newErr

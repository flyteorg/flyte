        ...
        <T::ResponseBody as Body>::Error: Into<StdError> + Send,
    {
        ...
        > {
            ...
            req.extensions_mut()
                .insert(
                    GrpcMethod::new(
                        "flyteidl2.environment.EnvironmentService",
                        "ListEnvironments",
                    ),
                );
...
pub mod environment_service_server {
    ...
    #[async_trait]
    pub trait EnvironmentService: Send + Sync + 'static {
...
*/
    ...
    #[derive(Debug)]
    pub struct EnvironmentServiceServer<T: EnvironmentService> {
        inner: Arc<T>,
    ...
    impl<T: EnvironmentService> EnvironmentServiceServer<T> {
        pub fn new(inner: T) -> Self {
    ...
    impl<T: EnvironmentService> Clone for EnvironmentServiceServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    impl<T: EnvironmentService> tonic::server::NamedService
    for EnvironmentServiceServer<T> {
        const NAME: &'static str = "flyteidl2.environment.EnvironmentService";
    }
}

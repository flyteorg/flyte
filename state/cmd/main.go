...
func serve(ctx context.Context) error {
	...
	logger.Infof(ctx, "Starting State Service")
	...
	// Create state client
	stateClient := k8s.NewStateClient(k8sClient, cfg.Kubernetes.Namespace, cfg.WatchBufferSize)
	logger.Infof(ctx, "Kubernetes client initialized for namespace: %s", cfg.Kubernetes.Namespace)

	// Start watching TaskActions
	...
	mux.HandleFunc("/readyz", func(w http.ResponseWriter, r *http.Request) {
		// State service is always ready (no database to check)
		w.WriteHeader(http.StatusOK)
		_, _ = w.Write([]byte("OK"))
	})
	...
	go func() {
		logger.Infof(ctx, "State Service listening on %s", addr)
	...
	logger.Infof(ctx, "State Service stopped")
	return nil
}

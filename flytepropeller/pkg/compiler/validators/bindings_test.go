...
func TestValidateBindings(t *testing.T) {
	...
	t.Run("Union Literal to Incompatible Union Binding", func(t *testing.T) {
		...
		assert.False(t, ok)
		assert.Equal(t, "MismatchingTypes", string(compileErrors.Errors().List()[0].Code()))
		assert.Contains(t, "Code: MismatchingTypes, Node Id: node1, Description: Variable [x]"+
	...
	t.Run("Union Promise Ambiguous", func(t *testing.T) {
		...
		assert.False(t, ok)
		assert.Equal(t, "MismatchingTypes", string(compileErrors.Errors().List()[0].Code()))
		assert.Contains(t, "Code: MismatchingTypes, Node Id: node1, Description: The output variable 'n2.n2_out'"+

...
func TestValidateBindings(t *testing.T) {
	...
	t.Run("Union Literal to Incompatible Union Binding", func(t *testing.T) {
		...
		assert.False(t, ok)
		assert.Equal(t, "MismatchingTypes", string(compileErrors.Errors().List()[0].Code()))
		assert.Equal(t, "Code: MismatchingTypes, Node Id: node1, Description: Variable [x] (type [union_type:{variants:{simple:INTEGER structure:{tag:\"int\"}}}]) doesn't match expected type [union_type:{variants:{simple:INTEGER structure:{tag:\"int_other\"}}}].", compileErrors.Errors().List()[0].Error())
	})

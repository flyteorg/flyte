...
func (in *InputData) UnmarshalJSON(b []byte) error {
	in.InputData = &core.InputData{}
	return jsonpb.Unmarshal(bytes.NewReader(b), in.InputData)
}

func (in *InputData) MarshalJSON() ([]byte, error) {
	if in == nil || in.InputData == nil {
		return nilJSON, nil
	}

	var buf bytes.Buffer
	if err := marshaler.Marshal(&buf, in.InputData); err != nil {
		return nil, err
	}

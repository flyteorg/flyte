...
func UpdateNodeStatus(np v1alpha1.NodePhase, p handler.PhaseInfo, n interfaces.NodeStateReader, s v1alpha1.ExecutableNodeStatus, clearStateOnAnyTermination bool) {
	// We update the phase and / or reason only if they are not already updated
	if np != s.GetPhase() || p.GetReason() != s.GetMessage() {
		s.UpdatePhase(np, ToK8sTime(p.GetOccurredAt()), p.GetReason(), clearStateOnAnyTermination, p.GetErr())
	}
	// Update TaskStatus

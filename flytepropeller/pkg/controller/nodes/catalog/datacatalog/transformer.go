...
func GenerateTaskOutputsFromArtifact(id core.Identifier, taskInterface core.TypedInterface, artifact *datacatalog.Artifact) (*core.LiteralMap, error) {
	...
	for _, artifactData := range artifactDataList {
		...
		if !validators.IsInstance(artifactData.Value, expectedVarType) {
			return nil, fmt.Errorf("unexpected artifactData: [%v] val: [%v] does not match any task output type: [%v]", artifactData.Name, artifactData.Value, expectedVarType)
		}

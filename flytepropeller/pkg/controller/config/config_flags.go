...
func (cfg Config) GetPFlagSet(prefix string) *pflag.FlagSet {
	...
	cmdFlags.Bool(fmt.Sprintf("%v%v", prefix, "node-config.clear-previous-error"), defaultConfig.NodeConfig.ClearPreviousError, "When using the fail after executable nodes complete mode many nodes in the workflow can fail. This option causes the previous error to be cleared every time a new error occurs. This helps reduce the size of workflow state in etcd.")

...
func (cfg Config) GetPFlagSet(prefix string) *pflag.FlagSet {
	...
	cmdFlags.Bool(fmt.Sprintf("%v%v", prefix, "node-config.disable-input-file-writes"), defaultConfig.NodeConfig.DisableInputFileWrites, "Disable writing input files to storage. This is useful for v2 nodes that are guaranteed to have inputs files already created")
	cmdFlags.Bool(fmt.Sprintf("%v%v", prefix, "node-config.persist-cache-status"), defaultConfig.NodeConfig.PersistCacheStatus, "Persist cache status in the node status. This is needed to persist cache status in v2 as part of the action status")

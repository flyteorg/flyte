...
func (cfg Config) GetPFlagSet(prefix string) *pflag.FlagSet {
	...
	cmdFlags.Bool(fmt.Sprintf("%v%v", prefix, "node-config.bypass-parallelism-check-for-workflow-nodes"), defaultConfig.NodeConfig.BypassParallelismCheckForWorkflowNodes, "Bypass max parallelism checks for workflow (subworkflow/launchplan) nodes that are already running")
	...
	cmdFlags.String(fmt.Sprintf("%v%v", prefix, "array-node-config.max-delta-timestamp"), defaultConfig.ArrayNode.MaxDeltaTimestamp.String(), "Maximum delta timestamp between ArrayNode start and an individual subNode start.")

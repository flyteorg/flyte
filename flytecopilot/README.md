...
Flyte CoPilot provides a uploader that understand Flyte Metadata Format as specified in FlyteIDL and make it possible to run arbitrary containers in Flyte.
This is achieved using `flyte-copilot` a binary that runs in 2 modes, -_Downloader_ - Downloads the metadata and any other data (if configured) to a provided path. In kubernetes this path could be a shared volume.

- _Uploader_ - Monitors the process and uploads any data that is generated by the process in a prescribed path/
...
As a uploader process, that runs in parallel with the main container/process, the goal is to
...
Solution 2:
Create a protocol. Main container will exit and write a \_SUCCESS file to a known location - problem in the case of oom or random exits. Uploader will be stuck. We could use a timeout? and in the uploader just kill the pod, when the main exits unhealthy?
